...
An orchestrator may assign qualifying ordinary work under this default. That is coordination, not delegated operator authority. Creating a lane worktree through the sanctioned repository worktree script and dispatching qualifying ordinary work into the worktree owned by that lane is coordination too, not a write outside the checkout or an action on another lane, and needs no operator sentence. It may route a privileged action only when the receiving lane already has direct operator authorization for that exact action in its own session.
