...
describe("opt-in resolution: env unset → file is the single source", () => {
  ...
  test("grok is an allowlisted client in the file", async () => {
    const r = await runWatchdog({ file: "codex,gemini,claude,cursor,agy,kimi,grok\n" });
    expect(r.code).toBe(0);
    expect(r.gatePassed).toBe(true);
  });
  test("opencode is an allowlisted client in the file", async () => {
    const r = await runWatchdog({ file: "codex,gemini,claude,cursor,agy,kimi,grok,opencode\n" });
    expect(r.code).toBe(0);
    expect(r.gatePassed).toBe(true);
  });
