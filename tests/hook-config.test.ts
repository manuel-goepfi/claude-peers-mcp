...
describe("canonical hook configuration", () => {
  test("moves a stale managed hook without changing an unrelated bucket matcher", () => {
    ...
    expect(classifyClientHooks(installed as Record<string, unknown>, "codex", repoRoot).exact).toBe(5);
  });
  ...
  test("classification requires exact matcher and does not confuse missing specs", () => {
    ...
    expect(classifyClientHooks(oneHook, "codex", repoRoot)).toEqual({ expected: 5, exact: 1, stale: 0, missing: 4 });
    ...
    expect(classifyClientHooks(wrongMatcher as Record<string, unknown>, "codex", repoRoot)).toEqual({ expected: 5, exact: 0, stale: 1, missing: 4 });
  });
});
